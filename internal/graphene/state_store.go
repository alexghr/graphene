...
func (g Git) writeStateLocked(state State) error {
	if state.Operation != nil && state.Pending != nil {
		return fmt.Errorf("graphene state contains both an operation and legacy pending state")
	}
	if err := state.Operation.validate(); err != nil {
		return err
	}
...
func decodeStateFile(data []byte) (stateFile, error) {
	...
	if file.Operation != nil && file.Pending != nil {
		return stateFile{}, fmt.Errorf("graphene state contains both an operation and legacy pending state")
	}
	if err := file.Operation.validate(); err != nil {
		return stateFile{}, err
	}
	return file, nil
}
...
func decodeLegacyState(raw string) (State, error) {
	...
	if state.Operation != nil {
		return State{}, fmt.Errorf("parse %s: legacy state contains a file-backed operation", stateConfigKey)
	}
	return state, nil
}
...
func writeStateFileAtomic(path string, file stateFile) error {
	...
	if err := os.Chmod(dir, 0o700); err != nil {
		return fmt.Errorf("secure graphene state directory: %w", err)
	}
