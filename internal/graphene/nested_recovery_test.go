...
func TestRecoveryLeavesNestedCheckoutsAlone(t *testing.T) {
	for _, command := range []string{"sync", "restack"} {
		for _, action := range []string{"success", "continue", "abort"} {
			t.Run(command+"/"+action, func(t *testing.T) {
				...
				runGit(t, baseDir, "update-index", "--cacheinfo", "160000,"+nextModule+",module")
				basePath := "file.txt"
				if action == "success" {
					basePath = "base-extra"
				}
				writeFile(t, baseDir, basePath, "base update\n")
				runGit(t, baseDir, "add", basePath)
				runGit(t, baseDir, "commit", "-m", "Update base")
				if command == "sync" {
				...
				if action == "success" {
					if code != 0 {
						t.Fatalf("%s failed: %s", command, stderr)
					}
				} else {
					...
					assertBranchParent(t, repo.dir, "stack/one", map[string]string{"sync": "main", "restack": "target"}[command])
				...
				if nestedState(t, module) != beforeModule || nestedState(t, nested) != beforeNested {
					t.Fatal("operation changed nested checkout, staging, or edits")
				}
				wantModule := nextModule
				if action == "abort" {
					wantModule = originalModule
				...
				if got := runGit(t, repo.dir, "rev-parse", "HEAD:module"); got != wantModule {
					t.Fatalf("recorded gitlink = %s, want %s", got, wantModule)
				}
				if readState(t, repo.dir).Pending != nil {
					t.Fatal("operation left pending recovery")
				}
			})
		}
	}
