...
func TestSyncFetchesOncePerRemote(t *testing.T) {
	...
	for _, separateRemote := range []bool{false, true} {
		t.Run(fmt.Sprintf("separate-remote=%t", separateRemote), func(t *testing.T) {
			t.Parallel()
			...
			code, stdout, stderr := repo.runGraphene(t, "sync", "--all", "--assume-merged")
			if code != 0 {
				t.Fatalf("sync exited %d\n%s\n%s", code, stdout, stderr)
			}
			if refExists(t, repo.dir, "refs/heads/stack/one") || refExists(t, repo.dir, "refs/heads/stack/two") {
...
func TestSyncFetchPrefixLeavesOtherStackUntouched(t *testing.T) {
	...
	t.Run("custom prefix", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal("fetch included an unrelated remote object outside the configured prefix")
		}
	})
}
...
func TestSyncChecksUpstreamsOutsidePrefix(t *testing.T) {
	t.Parallel()
	for _, namespace := range []string{"refs/changes/"} {
